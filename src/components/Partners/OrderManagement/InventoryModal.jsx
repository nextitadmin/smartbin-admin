...
const getStatusChipClasses = (status) => {
    ...
    switch (status?.toLowerCase()) {
        ...
        case 'inventory': return `${baseClasses} bg-zinc-100 text-zinc-800`;
...
const ActionPopover = ({ onAction }) => {
    return (
        <div className="absolute top-full left-0 mt-2 w-64 bg-white border border-zinc-200 rounded-lg text-sm z-20 shadow-lg">
            <button onClick={() => onAction('schedule')} className="w-full text-left px-4 py-3 hover:bg-zinc-50 rounded-b-lg">
...
const InventoryOrderDetailsContent = ({ order, onClose, onTransition }) => {
...
// --- MODAL WRAPPER ---
const InventoryModal = ({ isOpen, order, onClose, onTransition }) => {
    ...
    return (
        ...
        >
            {isLoading ? (
                ...
                <InventoryOrderDetailsContent order={order} onClose={onClose} onTransition={onTransition} />
            )}
        </div>
    );
};
