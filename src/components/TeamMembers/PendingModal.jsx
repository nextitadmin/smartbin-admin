...
const ActionPopover = ({ onAction, isProcessing }) => {
    return (
        <div className="absolute top-full left-0 mt-2 w-64 bg-white border border-zinc-200 rounded-lg text-sm z-20 shadow-lg">
            <button onClick={() => onAction('finish-delivery')} disabled={isProcessing} className="w-full text-left px-4 py-3 hover:bg-zinc-50 rounded-lg disabled:opacity-50">
                <p className="font-semibold text-zinc-800">Finish delivery</p>
                <p className="text-zinc-500">Mark order as delivered</p>
            </button>
...
const PendingOrderDetailsContent = ({ order, onClose, onTransition }) => {
    ...
    const deliverOrder = useTeamOrderStore(state => state.deliverOrder);
    ...
    const handleAction = async (action) => {
        ...
        } else if (action === 'finish-delivery') {
    ...
    const handleDeliverySubmit = async (deliveryData) => {
        ...
        try {
            const idToUse = order.applicationId || order.id;
            await deliverOrder(idToUse, deliveryData);
            if (onTransition) {
                onClose();
            }
        } catch (error) {
...
// --- SKELETON LOADER ---
const ModalSkeleton = () => (
    <div className="bg-white p-4 sm:p-6 rounded-lg border border-zinc-200 w-full max-w-3xl mx-auto animate-pulse">
        {/* Header Skeleton */}
        <div className="flex justify-between items-center pb-4 border-b border-zinc-200">
            <div className="flex items-center gap-4">
