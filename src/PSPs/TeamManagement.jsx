...
const TeamManagement = () => {
  ...
  // store PSP id for API calls (obtained from fetched members or context)
  ...
  const handleAddRoleSubmit = async (e) => {
    ...
    const validModules = modules.filter(
      (module) => module.name && module.privileges.length > 0,
    );
    if (validModules.length === 0) {
      showNotification(
        "Please select at least one module with privileges.",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      console.error(
        `Error ${isEditRoleMode ? "updating" : "creating"} role:`,
        error,
      );
      showNotification(
        `An error occurred while ${isEditRoleMode ? "updating" : "creating"} the role.`,
        "error",
      );
    } finally {
  ...
  const handleDeactivateMember = async () => {
    ...
    try {
      ...
      const pspIdFromMember = memberObj?.pspId;
      console.log("handleDeactivateMember found pspIdFromMember", pspIdFromMember, typeof pspIdFromMember);
      if (!pspIdFromMember || typeof pspIdFromMember !== 'string') {
        showNotification("Missing or invalid PSP ID", "error");
        return;
      }
      const pspIdString = String(pspIdFromMember);

      // Make API call to update status via PSPs team endpoint
      console.log("deactivating member", memberId, "pspId", pspIdString);
      const deactivateUrl = `/lawma/psps/team/members/${memberId}?pspId=${pspIdString}`;
      const deactivatePayload = {
        status: "deactivated",
        pspId: pspIdString,
        psp_id: pspIdString,
      };
      console.log("DEACTIVATE url", deactivateUrl);
      console.log("DEACTIVATE payload", deactivatePayload);
      const { data } = await api.put(deactivateUrl, deactivatePayload);
      ...
      if (error.response) {
        ...
        if (status === 404) {
          showNotification(
            "Member not found. It may have already been deactivated.",
            "error",
          );
        } else if (status === 403) {
          showNotification(
          ...
          showNotification(
            "Cannot deactivate member. Please check for dependencies.",
            "error",
          );
        } else if (status >= 500) {
        ...
        // Other error
        showNotification(
          "An unexpected error occurred while deactivating the member.",
          "error",
        );
      }
    } finally {
  ...
  const handleAddMemberSubmit = async (e) => {
    ...
    console.log("handleAddMemberSubmit called, pspId state=", pspId);
    ...
    try {
      // determine PSP id for this operation; prefer explicit state but fall back
      let effectivePspId = pspId || editingMember?.pspId;
      if (isEditMode) {
        // --- UPDATE MEMBER ---
        const memberId = editingMember._id || editingMember.id;
        const originalMember = teamMembers.find(
          (m) => m._id === memberId || m.id === memberId,
        );

        // note: member object stores pspId property from fetchTeamMembers
        effectivePspId = effectivePspId || originalMember?.pspId;

        if (!effectivePspId) {
          showNotification("Missing PSP ID", "error");
          return;
        }

        console.log("about to update member with pspId", effectivePspId, typeof effectivePspId);
        const updateUrl = `/lawma/psps/team/members/${memberId}?pspId=${String(effectivePspId)}`;
        const updatePayload = {
          name: newName,
          email: newEmail,
          phone_number: newPhone,
          role: newRole.toLowerCase().trim(),
          status: newStatus.toLowerCase().trim(),
          pspId: String(effectivePspId),
          psp_id: String(effectivePspId),
        };
        ...
        // --- ADD NEW MEMBER ---
        if (!pspId) {
          showNotification("Unable to add member: missing PSP ID", "error");
          return;
        }
        console.log("creating member with pspId", pspId, typeof pspId);
        const createPayload = {
          name: newName,
          email: newEmail,
          phone_number: newPhone,
          role: newRole.toLowerCase().trim(),
          pspId: String(pspId),
          psp_id: String(pspId), // some endpoints may expect snake_case
        };
        console.log("ADD payload", createPayload);
        const { data } = await api.post(
          `/lawma/psps/team/members?pspId=${encodeURIComponent(String(pspId))}`,
          createPayload,
        );

        showNotification("Team member added successfully!", "success");

        // Re-fetch or manually map as we did before
        fetchTeamMembers();
      }
    } catch (error) {
    ...
    // debugging: inspect PSP info if needed
    console.log("editingMember:", editingMember);
    console.log("pspId:", editingMember?.pspId);
  };
  ...
  const handleEditMemberClick = () => {
    const memberToEdit = teamMembers.find(
      (m) => m.id === currentMemberId || m._id === currentMemberId,
    );
    ...
    if (memberToEdit) {
      ...
      // Ensure the status is normalized for the dropdown (e.g., "active" -> "Active")
      const rawStatus = memberToEdit.status || "Active";
      setNewStatus(
      ...
      showNotification("Could not find member details", "error");
    }
  }; // Moved the brace here to correctly wrap the logic
  ...
  const fetchTeamMembers = async () => {
    setIsLoading(true);
    try {
      const { data: teamData } = await api.get("/lawma/psps/team/members");
      const rawMembers = Array.isArray(teamData)
        ? teamData
        : teamData?.data || [];
      ...
      const members = rawMembers.map((member) => ({
        ...
        status: member.status
          ...
          : "Active",
        // PSP information (could be nested) always stringified
        pspId: String(member.psp_id || member.psp_details?._id || ""),
        pspName: member.psp_details?.company_name || "",
      }));
      setTeamMembers(members);
      // record the PSP id for future API operations if not already set
      if (members.length && !pspId) {
        const newPspId = String(members[0].pspId);
        console.log("fetchTeamMembers setPspId", newPspId);
        setPspId(newPspId);
      }

      // Fetch roles
      try {
        const { data: rolesData } = await api.get("/psps/roles");
        ...
        setRoles([]); // Fallback to empty array, not demoRoles
      }
    } catch (error) {
      console.error("Error fetching team members:", error);
      setTeamMembers([]); // Fallback to empty array, not demoTeamMembers
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const totalPages = Math.ceil(
    (activeTab === "members" ? filteredMembers.length : filteredRoles.length) /
      itemsPerPage,
  );
  ...
  const handleExportData = async () => {
    ...
    try {
      ...
      showNotification(
        `${activeTab === "members" ? "Members" : "Roles"} data exported successfully!`,
        "success",
      );
    } catch (error) {
          ...
          <main className="p-4 md:px-4">
            <div className="p-4 md:p-8 font-sans">
              ...
              {notification.visible && (
                <div
                  className={`fixed top-5 right-5 z-50 p-4 rounded-md shadow-lg text-white flex items-center space-x-2
              ...
              <header className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center">
                <div>
                  ...
                  <p className="text-zinc-500 text-lg font-light">
                    Invite and manage admin team members here
                  </p>
                </div>
                <button
                    ...
                    <table className="w-full min-w-[900px] m-4 table-auto border-collapse">
                      <thead className="border-b border-zinc-200">
                        {activeTab === "members" ? (
                          <tr>
                            <th
                              className="lg:p-6 p-3 text-left text-xs font-medium text-zinc-500 uppercase tracking-wider w-12 cursor-pointer"
                            ...
                            <th
                              className="lg:p-6 p-3 text-left text-sm font-medium text-zinc-500 uppercase tracking-wider cursor-pointer"
                              ...
                              >
                                ...
                                <td className="lg:p-6 p-3">
                                  <div className="relative">
                                    <button
                                      onClick={(e) =>
                                        handleRowActionClick(role.id, e)
                                      }
                                      type="button"
                                      className="p-1 text-zinc-500 hover:text-zinc-700"
                                    >
                    ...
                    <form onSubmit={handleAddMemberSubmit}>
                      ...
                      <div className="mb-4">
                        ...
                        <select
                          ...
                          className="w-full lg:p-4 p-2 border border-zinc-300 rounded-2xl focus:ring focus:border-green-700"
                        >
                      ...
                      <div className="space-y-4">
                        ...
                        {modules.map((_, index) => (
                          <div key={index} className="flex items-center gap-3">
                            ...
                            <div className="w-1/2">
                              <select
                                className=" border border-zinc-300 rounded-lg focus:ring focus:outline-none focus:ring-green-700 focus:border-green-700 w-full px-2 py-2 text-lg"
                            ...
                            <div className="w-1/2 flex items-center">
                              <div className="flex gap-x-4 gap-y-2">
                                {["View", "Edit", "Export", "Something"].map(
                                  (priv, i) => (
                                    <label
                                      key={i}
