...
const SmartBinReport = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            if (!id) {
                setReportData({ items: [] });
                setNotification({ show: true, type: 'error', message: 'No report ID provided.' });
                setIsLoading(false);
                return;
            }
            ...
            if (data.success) {
                ...
                const recordsArray = report.data.records || report.data.binDisposalAnalytics || [];
                const reportsData = recordsArray.map((item, index) => ({
                    sn: index + 1,
                    id: item.orderId || item._id,
                    orderId: item.orderId || item._id,
                    date: item.dateRequested || report.startDate,
                ...
                setReportData({
                    ...
                    reportPeriod: formatPeriodArrow(report.period) || " ",
                    generatedDate: report.createdAt,
                    totalBinsOrdered: recordsArray.length,
                    items: reportsData
                });
            }
            setIsLoading(false);
