...
import PaymentsReportSkeletonLoader from '../components/PSPs/PaymentsReportSkeletonLoader';
...
const PaymentReportPage = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const { data } = await api.get(`/corporate/reports/${id}`);
            if (data.success) {
                const report = data.data.report;
                // Format transactions data based on API response
                const transactionsData = []; // Populate based on actual API response if available
                
                setReportData({
                    title: report.reportName || 'Payment Report',
                    generatedDate: formatGenerationDate(report.createdAt),
                    dateRange: formatPeriodArrow(report.period),
                    totalPaymentMade: 0,
                    smartBinPurchaseTotal: 0,
                    wasteDisposalTotal: 0,
                    walletFundingTotal: 0,
