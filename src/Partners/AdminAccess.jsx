...
import { PlusIcon, MagnifyingGlassIcon, XMarkIcon, EllipsisVerticalIcon, CheckCircleIconSolid, ExclamationTriangleIconSolid, LoadingSpinnerIcon, SortIcon, ChevronLeftIcon, ChevronRightIcon, SearchIcon } from '../components/icons';
...
const AdminAccess = () => {
    ...
    const handleDeactivateMember = async () => {
        ...
        try {
            ...
            showNotification('An error occurred while deactivating the member.', 'error');
        } finally {
    ...
    const handleEditMemberClick = () => {
        const memberToEdit = teamMembers.find(member => member.id === currentMemberId);
        if (memberToEdit) {
            setEditingMember(memberToEdit);
            setNewName(memberToEdit.name);
            setNewEmail(memberToEdit.email);
            setNewPhone(memberToEdit.phone);
    ...
    const filteredRoles = useMemo(() => {
        let filtered = roles.filter(role =>
            ...
            role.members.toString().includes(searchTerm)
        );
    ...
    const closeModal = () => {
        ...
        setIsEditRoleMode(false);
        setEditingRole(null);
        // Don't change activeTab when closing modals - stay on current tab
        // The activeTab should remain unchanged when closing any modal
    };
                        ...
                        ${activeTab === 'roles-and-privileges'
                            ...
                            <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
                                <div className="relative flex-grow max-w-lg">
                                    ...
                                    />
                                    <SearchIcon className="absolute left-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-green-800" />
                                                ...
                                                {activeTab === 'members' ? (
                                                    <tr>
                                                        <th className="lg:p-6 p-3 text-left text-xs font-medium text-zinc-500 uppercase tracking-wider w-12 cursor-pointer" onClick={() => handleSort('id')}>
                                                            <div className="flex items-center" direction={sortConfig.key === 'id' ? sortConfig.direction : null}>
                                                                S/N
                                                        ...
                                                        <th className="lg:p-6 p-3 text-left text-sm font-medium text-zinc-500 uppercase tracking-wider cursor-pointer" onClick={() => handleSort('name')}>
                                                            <div className="flex items-center" direction={sortConfig.key === 'name' ? sortConfig.direction : null}>
                                                                Name
                                                ...
                                                <div className="mb-6">
                                                    <p className="text-zinc-700 text-lg">
                                                        ...
                                                        <span className="font-semibold text-zinc-800">
                                                            {actionType === 'member'
                                                                ? teamMembers.find(member => member.id === currentMemberId)?.name
