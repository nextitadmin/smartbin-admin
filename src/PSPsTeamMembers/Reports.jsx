...
// List endpoint expects these `type` values:
// revenue, payment-history, waste-disposed, waste-pickup
const REPORT_TYPES = [
    ...
    { name: 'Bin Delivered', value: 'smartbin-delivered' },
    { name: 'Unpaid Bills', value: 'unpaid-bills' },
    { name: 'Registered Users', value: 'users-registration' },
];
...
const ReportsPage = () => {
    ...
    // Backend supports pagination; keep defaults for now.
    ...
    const fetchReportsAPI = useCallback(async () => {
        setIsLoading(true);
        try {
            const response = await api.get('/lawma/psp/reports', {
                params: {
                    type: filterReportType !== 'All' ? filterReportType : undefined,
            ...
            console.log("Reports data fetched successfully", response.data);
            
            const payload = response?.data;

            // Support a couple of common shapes:
            const rawReports = payload?.data?.reports ?? payload?.data ?? [];

            if (payload?.success && Array.isArray(rawReports)) {
                const reportList = rawReports.map((item) => ({
                    id: item._id ?? item.id,
                    reportType: item.type,
                    reportTitle: item.reportName ?? item.report_title ?? item.title,
                    period: formatPeriodArrow(item.period),
                    generationDate: item.createdAt ?? item.created_at,
                }));
                setReports(reportList);
            } else {
                throw new Error(payload?.message || 'Unexpected reports response shape');
            }
        } catch (error) {
    ...
    }, [filterEndDate, filterReportType, filterStartDate, limit, page, searchTerm]);
    useEffect(() => {
        fetchReportsAPI();
    }, [fetchReportsAPI]);
    ...
    const processedReports = useMemo(() => {
        ...
        if (filterStartDate) {
            ...
            filtered = filtered.filter(report => new Date(report.generationDate) >= from);
        }
        if (filterEndDate) {
            const to = new Date(filterEndDate);
            // include entire end day
            to.setHours(23, 59, 59, 999);
            filtered = filtered.filter(report => new Date(report.generationDate) <= to);
        }
        if (sortConfig.key) {
            filtered.sort((a, b) => {
        ...
        return filtered;
    }, [reports, searchTerm, filterReportType, filterStartDate, filterEndDate, sortConfig]);
    ...
    // formatPeriod was unused; removed to satisfy eslint (no-unused-vars).
